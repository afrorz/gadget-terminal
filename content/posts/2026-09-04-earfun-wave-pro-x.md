---
title: EarFun「Wave Pro X」発売、デュアル同軸ドライバーとQualcomm新チップで最大90時間再生
seo_title: EarFun Wave Pro Xが発売、129.99ドル
slug: earfun-wave-pro-x
keyword: Wave Pro X
category: smartphone
date: 2026-09-04
kicker: EarFunが、40mmと10mmのドライバーを同軸配置した新設計「DCAA」を積むワイヤレスヘッドホン「Wave Pro X」を129.99ドルで発売した。Qualcommの新しいサウンド向けチップとBluetooth 6.0を採用し、最大90時間の再生に対応するとしている。
tags:
- EarFun
- Wave Pro X
- ワイヤレスヘッドホン
- Qualcomm
- ノイズキャンセリング
x_hook: ドライバーを2つ同じ軸に並べる設計で、音のズレを無くしにいったヘッドホン。バッテリーは最大90時間。
embeds: []
images:
- url: "https://api.myearfun.com/image/product/808n42741rpoazs5gdu.png"
  caption: 本体正面。折りたたみ式のオーバーイヤー型で、右ハウジングに電源・音量ボタンを配置
- url: "https://api.myearfun.com/image/product/683ihapwcnkhgjm3tqx.jpg"
  caption: 本体単体カット。CES 2026関連の受賞バッジとともに掲載されている構図
- url: "https://api.myearfun.com/image/product/696pmiqo1hvarstr219.jpg"
  caption: DCAA(デュアル同軸音響構造)の分解図。40mm DLC振動板と10mm LCP振動板、Qualcomm QCC3095チップの配置が確認できる
credit: EarFun公式サイト(myearfun.com)
faq:
- q: 日本で買えますか
  a: 掲載時点ではEarFun公式サイトとAmazonでの発売(2026年9月3日)のみ案内されており、国内正規販売の告知は確認できていません。
- q: いくらですか
  a: 129.99ドルです(掲載時点、米国での価格)。
- q: 技適は取得していますか
  a: 掲載時点でWave Pro X単体の技適取得状況は確認できていません。同社の前モデル「EarFun Wave Pro」は国内の正規代理店経由で販売された実績があります。
- q: 何が新しいのですか
  a: 40mm DLC動的ドライバーと10mm LCPドライバーを同じ軸上に並べる「DCAA(デュアル同軸音響構造)」と、周囲の騒音レベルに応じてノイズキャンセリングの強さをリアルタイムに調整する「Adaptive Hybrid ANC」です。QualcommのSnapdragon S3 Gen 1 Sound Platform(QCC3095)を採用し、Bluetooth 6.0やAuracastにも対応します。
alternatives:
- name: EarFun Wave Pro ワイヤレスヘッドホン(Amazon.co.jp・EarFun公式店)
  why: 同じEarFunのANC搭載ワイヤレスヘッドホンで、VGP 2024 SUMMER受賞歴があり国内の正規代理店から今日購入できる。Wave Pro Xと同じDCAA・Bluetooth 6.0ではないが、EarFunのノイズキャンセリング機の実力を国内サポート付きで試せる。
  url: https://www.amazon.co.jp/dp/B0CSDKW6DN
  merchant: amazon
  image: "https://thumbnail.image.rakuten.co.jp/@0_mall/earfun/cabinet/11055449/imgrc0095763400.jpg?_ex=400x400"
sources:
- title: EarFun Unveils Wave Pro X Dual-Coaxial Headphones With Qualcomm ANC Technology and Auracast
  url: https://www.techpowerup.com/352318/earfun-unveils-wave-pro-x-dual-coaxial-headphones-with-qualcomm-anc-technology-and-auracast
  publisher: TechPowerUp
- title: EarFun Wave Pro X
  url: https://www.myearfun.com/headphones/earfun-wave-pro-x-hi-res-hybrid-anc-true-wireless-headphones-black
  publisher: EarFun公式サイト
---

EarFunが2026年9月3日、ワイヤレスヘッドホン新製品「Wave Pro X」を129.99ドルで発売した。EarFun公式サイトとAmazonで即日発売されている。目玉は40mmと10mmのドライバーを1つの音響中心に揃えて配置する新設計「DCAA(Dual-Coaxial Acoustic Architecture)」と、Qualcommの新しいサウンド向けチップの採用だ。

## スペック

| 項目 | 内容 |
| --- | --- |
| 価格 | 129.99ドル(掲載時点、米国) |
| チップ | Qualcomm Snapdragon S3 Gen 1 Sound Platform(QCC3095)、Bluetooth 6.0 |
| ドライバー | DCAA方式、40mm DLC動的ドライバー+10mm LCPドライバーを同軸配置、AIクロスオーバー同期 |
| ノイズキャンセリング | Adaptive Hybrid ANC、8マイクのハイブリッド構成、USB/AUX有線接続でもANC有効 |
| 対応コーデック | Qualcomm aptX Adaptive、aptX Lossless、LDAC、Hi-Res Audio認証、ロスレスオーディオ対応 |
| バッテリー | 最大90時間(LE Audioの省電力性を活用) |
| その他 | Auracast、Google Fast Pair、マルチポイント接続、専用アプリでのゲーム向け低遅延モード |

EarFunによれば、DCAAは2つのドライバーを別々の経路で鳴らすのではなく、同一の音響中心から発音させることで低域から高域までの位相ズレを抑える設計だという。ANCは周囲の騒音環境をリアルタイムに解析し、電車内やオフィスのような騒がしい環境では強く、静かな環境では弱くかける「適応型」だとしている。

## 日本から見るとどうか

**国内発売の告知は掲載時点で確認できていない。** 公式サイトとAmazonでの発売が案内されているのみで、国内正規代理店の取り扱いやEarFun公式の日本向けページでの案内は見当たらなかった。技適の取得状況もWave Pro X単体では確認できていない。

**ただしEarFunは国内実績のあるブランドではある。** 前モデルの「EarFun Wave Pro」はVGP 2024 SUMMERを受賞し、国内の正規代理店(Amazon.co.jpのEarFun公式店)から購入できる。今回のWave Pro Xについても、今後同様に国内向け販売が始まる可能性はあるが、掲載時点では未確定として扱いたい。

**価格はドル建て。** 129.99ドルは為替により円換算額が変動し、Amazon.co.jpでの取り扱いが始まった場合も米国価格とは異なる可能性がある。

**今すぐ試したいなら。** 同じDCAA・Bluetooth 6.0搭載機ではないが、EarFunの適応型ANCを国内サポート付きで体験したい場合は、前モデルのWave Proが選択肢になる。
